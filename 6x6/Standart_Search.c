...
void Insert_Priority_Queue_GENERALIZED_A_Star(Node *const child, Queue **frontier, float alpha)
{
	...
	// Calculate the priority using the Generalized A* formula
	float priority = child->path_cost + alpha * child->state.h_n;
	...
	{
		// Find appropriate element according to ordered evaluation function values
		if (priority < (*frontier)->node->path_cost + alpha * (*frontier)->node->state.h_n)
		{
			new_queue->next = *frontier;
			*frontier = new_queue;
		}
		else
		{
			for (temp_queue = *frontier; temp_queue->next != NULL; temp_queue = temp_queue->next)
			{
				if (priority < temp_queue->next->node->path_cost + alpha * temp_queue->next->node->state.h_n)
				{
					new_queue->next = temp_queue->next;
					temp_queue->next = new_queue;
					return;
